...
void TestMSG(TestResult status)
{
	...
	EndPoint clientEP("localhost", 9892);
		...
		std::string rply = status.GetName();
		
		//set name
		if (TestResult::Status::PASS == status.GetStatus())
		{
			rply += ": Pass";
		}
		else if (TestResult::Status::FAIL == status.GetStatus())
		{
			rply += ": Fail";
		}
		else if (TestResult::Status::Fail_EXC == status.GetStatus())
		{
			rply += ": Fail EXC";
		}
		else if (TestResult::Status::NOT_RUN == status.GetStatus())
		{
			rply += ": NOT RUN";
		}
		else
		...
		double durration = status.GetDuration();
			rply += " " +std::to_string(durration);
		msg.name(rply);
